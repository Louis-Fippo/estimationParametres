...
#Date:02-12-2013
#But: fonction permettant d'estimer les paramètres r et sa
#ces paramètres seront éditer dans un fichier qui va préciser pour 
#chaque molécule les différents paramètres pour chaque action
#les actions peuvent être les activations ou les inhibitions
...
estimationParametres<-function()

{
  #ouverture du fichier à éditer pour l'inférence des paramètres
  con <- file("parametresEstimes.txt", open = "a+");
  
  #lecture du fichier des données
  genexpr=read.csv("sig_genes_result_temp.csv",sep="\t",h=T);
  donneesDiscrete=read.csv("sig_genes_result_temp_disFS.csv", sep="\t", h=T);
  ...
  for(i in 1:dimTab[1])
  {
    ...
    #on écrit le nom de la molecule concernée
    chaine=donneesDiscrete[i,2];
    #print(chaine);
    #print(class(as.character(chaine)));
    cat(as.character(chaine), file = con, sep=" ");
    #cat(donneesDiscrete[i,2], file = con, sep=" ");
    cat(": ", file=con, sep=" ");
    ...
    #pour chaque ligne(gene) on va discretiser
    ...
    for(j in vecteurCourantNom1)
    {
      ...
      if(NatureFront(donneesDepart[i,j],donneesDepart[i,pindex]))
      {
        ...
        if(signif[1]){
        cat("activation ", file = con, sep="  ");
        pprocess=signif[2]-1;nprocess=signif[2];#on détermine les processus précedent et suivant
        cat(pprocess, file=con, sep="  ");
        cat(" ", file=con, sep= " ");
        cat(nprocess, file=con, sep="  ");
        cat(" @", file=con, sep="  ");
        r=strsplit(j, '[^0-9]', perl = TRUE);
        for (element in r) {
          if(is.character(element)){
            cat(element, file=con, sep = "")
          }
        }
        cat("~50", file=con, sep= " ");
        cat("; ", file=con, sep= " ");
